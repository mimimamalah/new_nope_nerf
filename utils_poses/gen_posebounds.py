...
"""
Found this file in this github repository : 
https://github.com/raikuma/nope-nerf 
"""
...
Rt = c2w[:, :3, :]
hwf = np.array([[H], [W], [fx]]) * factor
hwf = np.tile(hwf, (c2w.shape[0], 1, 1))
poses = np.concatenate([Rt, hwf], axis=-1)

poses = np.concatenate([-poses[:, :, 1:2], poses[:, :, 0:1], poses[:, :, 2:]], axis=-1)

poses = np.reshape(poses, (c2w.shape[0], -1))
bd = cfg['rendering']['depth_range']
bds = np.tile(np.array([bd[0], bd[1]]), (c2w.shape[0], 1))
poses_bounds = np.concatenate([poses, bds], axis=1)
np.save(os.path.join(out_dir, 'poses_bounds.npy'), poses_bounds)
