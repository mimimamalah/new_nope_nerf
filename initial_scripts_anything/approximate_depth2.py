...
for file_name in os.listdir(source_dir):
    if file_name.endswith('.npz'):
        ...
        source_path = os.path.join(source_dir, file_name)
        model_file_name = os.path.join(output_dir, f'model_{file_name}.h5')
        scaler_X_file_name = os.path.join(output_dir, f'scaler_X_{file_name}.pkl')
        scaler_y_file_name = os.path.join(output_dir, f'scaler_y_{file_name}.pkl')

        # Ensure the model and scalers exist
        if os.path.exists(model_file_name) and os.path.exists(scaler_X_file_name) and os.path.exists(scaler_y_file_name):
            # Load the model and scalers for the current file
            model = load_model(model_file_name)
            scaler_X = joblib.load(scaler_X_file_name)
            scaler_y = joblib.load(scaler_y_file_name)
